    ...
    *,
    ...
    stmt = (
        ...
        .where(
            ...
            # Transfer-like rows stay visible here on purpose (upstream #582):
            # they are projections the user should see, they just never count
            # as P&L. Only ignored categories are filtered out.
            or_(
                RecurringTransaction.category_id.is_(None),
                Category.is_ignored.is_not(True),
            ),
    ...
    inst = await _get_installment_projections(
        session, workspace_id, range_start, range_end, force_effective=date_basis == "effective"
    )
