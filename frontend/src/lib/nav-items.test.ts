...
// Fork-only pages (cards, insights, debt planner, …) have no upstream
...
const moduleLinks = () =>
  navItems.filter((i) => i.type === 'link' && i.module !== undefined)
...
describe('nav catalog', () => {
  it('maps every link to a known module', () => {
    for (const item of navItems) {
      ...
      if (item.module === undefined) {
        expect(FORK_ONLY_KEYS).toContain(item.key)
        continue
      }
  ...
  it('has a link for every module in the catalog', () => {
    const covered = moduleLinks().map((i) => (i as { module: ModuleId }).module)
...
describe('visibleNavItems', () => {
  it('renders a personal workspace exactly as before modules existed', () => {
    ...
    // Upstream's module links, in order, with the fork's module-less pages
  ...
  it('drops every module link and its header when no module is on', () => {
    const result = visibleNavItems(navItems, none)
    // Only the fork's module-less pages survive; no module link remains.
    expect(linkKeys(result).every((k) => FORK_ONLY_KEYS.includes(k))).toBe(true)
    expect(result.some((i) => i.type === 'link' && i.module !== undefined)).toBe(false)
  })
